...
public interface CustomerInvoicesRepository extends BaseRepository<CustomerInvoices, Long>{
	  ...
	  */
	  ...
	  @Query("SELECT  isnull(sum(ci.invoiceAmount),0)  FROM CustomerInvoices ci where ci.customerId.customerId=?1 and ci.paymentStatus='N'")
	  Double getCustomerTotalInvoiceAmount(Long customerId);
	  ...
	  @Query("SELECT isnull(sum(ci.invoiceAmount),0)  FROM CustomerInvoices ci where ci.customerId.customerId=?1 and ci.paymentStatus='N' and CONVERT(DATE,ci.invoiceDueDate)<CONVERT(DATE, GETDATE())")
	  Double getCustomerUnpaidTotalInvoiceAmount(Long customerId);
