...
public abstract class AbstractNativeQuery<T extends NativeQuery<?, ?>, U>
	...
	public List<U> list() {
		...
		return executeList(Context.getCommandContext(), getParameterMap(), 0,
				Integer.MAX_VALUE);
	}
	...
	public Object execute(CommandContext commandContext) {
		if (resultType == ResultType.LIST) {
			return executeList(commandContext, getParameterMap(), 0,
					Integer.MAX_VALUE);
			...
			if (StringUtils.isNotBlank(ObjectUtils.toString(parameterMap
					.get("orderBy")))) {
				parameterMap.put("orderBy",
	...
	public U executeSingleResult(CommandContext commandContext) {
		List<U> results = executeList(commandContext, getParameterMap(), 0,
				Integer.MAX_VALUE);
		if (results.size() == 1) {
			return results.get(0);
		} else if (results.size() > 1) {
			throw new ActivitiException("Query return " + results.size()
